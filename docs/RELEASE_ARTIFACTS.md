# Exécutables multiplateformes de la version 1.3.0

Les exécutables sont publiés comme assets de la release GitHub associée au tag
immuable `v1.3.0`. Le tag référence le code source ; la release porte les archives
...
|---|---|---|
| `defta-librairie-1.3.0-windows-amd64.zip` | Windows 10/11 64 bits | AMD64 |
| `defta-librairie-1.3.0-linux-arm64.tar.gz` | Raspberry Pi OS 64 bits | ARM64, Raspberry Pi 3/4/5 |
| `defta-librairie-1.3.0-linux-armv7.tar.gz` | Raspberry Pi OS 32 bits | ARMv7, Raspberry Pi 2/3/4 |
...
`v1.3.0`, puis lancer l’exécution. Le workflow extrait le code du tag existant,
...
```sh
gh workflow run release-binaries.yml --ref develop -f tag=v1.3.0
gh run watch
```
...
```powershell
Get-FileHash .\defta-librairie-1.3.0-windows-amd64.zip -Algorithm SHA256
```
