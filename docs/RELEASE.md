...
CGO_ENABLED=1 go build -trimpath -tags fts5 \
  -o dist/defta-librairie-v1.3.0-linux-amd64 ./cmd
sha256sum dist/defta-librairie-v1.3.0-linux-amd64 \
  > dist/defta-librairie-v1.3.0-linux-amd64.sha256
```

Au lancement, fournir au minimum `VERSION=1.3.0`,
`BUILD_DATE=2026-09-15`, un `JWT_SECRET` issu du gestionnaire de secrets et
...
```sh
git tag -a v1.3.0 -m "Defta Librairie 1.3.0"
git show --no-patch --decorate v1.3.0
git push origin v1.3.0
```

Créer la release GitHub `v1.3.0` avec les notes de `CHANGELOG.md`, le binaire
