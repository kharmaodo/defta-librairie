...
CGO_ENABLED=1 go build -trimpath -tags fts5 \
  -o dist/defta-librairie-v1.2.0-linux-amd64 ./cmd
sha256sum dist/defta-librairie-v1.2.0-linux-amd64 \
  > dist/defta-librairie-v1.2.0-linux-amd64.sha256
```

Au lancement, fournir au minimum `VERSION=1.2.0`,
...
```sh
git tag -a v1.2.0 -m "Defta Librairie 1.2.0"
git show --no-patch --decorate v1.2.0
git push origin v1.2.0
```

Créer la release GitHub `v1.2.0` avec les notes de `CHANGELOG.md`, le binaire
