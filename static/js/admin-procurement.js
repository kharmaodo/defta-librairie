(() => {
  ...
  function fillLibraries(form, selected="") { if(!state.isRoot)return; fill(form.elements.libraryId,state.libraries,l=>l.name); form.elements.libraryId.value=selected||state.libraries[0]?.id||""; }
  ...
  function purchaseLibrary() { const f=document.querySelector("#purchase-form");return state.isRoot?f.elements.libraryId.value:""; }
  function scoped(items) { const id=purchaseLibrary();return id?items.filter(item=>item.libraryId===id):items; }
  function addPurchaseLine(line=null) { const fragment=document.querySelector("#purchase-line-template").content.cloneNode(true);const row=fragment.querySelector(".purchase-line"),select=row.querySelector("[name=bookId]");fill(select,scoped(state.books),b=>b.title);if(line){select.value=String(line.bookId);row.querySelector("[name=quantity]").value=line.quantity;row.querySelector("[name=unitCost]").value=line.unitCost;}row.addEventListener("input",updatePurchaseTotal);row.querySelector(".remove-purchase-line").onclick=()=>{row.remove();updatePurchaseTotal();};document.querySelector("#purchase-lines").append(row);updatePurchaseTotal();}
  function resetPurchaseScope() { const f=document.querySelector("#purchase-form");fill(f.elements.supplierId,scoped(state.suppliers).filter(s=>s.status==="ACTIVE"),s=>s.name);document.querySelector("#purchase-lines").replaceChildren();addPurchaseLine(); }
  function openPurchase(item=null) { const f=document.querySelector("#purchase-form"); f.reset(); fillLibraries(f,item?.libraryId); fill(f.elements.supplierId,scoped(state.suppliers).filter(s=>s.status==="ACTIVE"),s=>s.name); f.elements.id.value=item?.id||""; f.elements.version.value=item?.version||""; if(item)f.elements.supplierId.value=item.supplierId;document.querySelector("#purchase-lines").replaceChildren();(item?.lines?.length?item.lines:[null]).forEach(addPurchaseLine);document.querySelector("#purchase-error").hidden=true;document.querySelector("#purchase-dialog").showModal(); }
  async function init() {
