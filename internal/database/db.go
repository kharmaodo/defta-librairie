...
func SearchBooks(query string, offset, limit int) ([]models.Book, int, error) {
	...
	if query == "" {
		// Liste complète - table defta
		err = DB.QueryRow("SELECT COUNT(*) FROM defta").Scan(&total)
		if err != nil {
			return nil, 0, fmt.Errorf("count error (defta): %w", err)
		}
		...
		`, query).Scan(&total)
		if err != nil {
			return nil, 0, fmt.Errorf("fts count error: %w", err)
		}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("query preparation error: %w", err)
	}
	defer rows.Close()
