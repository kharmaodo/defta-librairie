...
type purchaseTransitionRequest struct {
...
func (h *PurchaseHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if decodeOwnerJSON(w, r, &input) != nil {
		writePurchaseError(w, services.ErrInvalidPurchase)
		return
	}
	claims, _ := auth.ClaimsFromContext(r.Context())
...
func (h *PurchaseHandler) transition(w http.ResponseWriter, r *http.Request, receive bool) {
	...
	var err error
	if receive {
		purchase, err = h.service.Receive(r.Context(), claims, r.PathValue("id"), request.Version)
	} else {
