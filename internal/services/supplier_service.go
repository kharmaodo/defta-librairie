...
func (s *SupplierService) List(ctx context.Context, claims *auth.Claims, requestedLibrary, query string,
	...
	if len([]rune(query)) > 160 || !validSupplierStatus(status, true) {
		return nil, 0, ErrInvalidSupplier
	}
...
func (s *SupplierService) Find(ctx context.Context, claims *auth.Claims, id string) (models.Supplier, error) {
	...
	if strings.TrimSpace(id) == "" {
		return models.Supplier{}, ErrInvalidSupplier
	}
	if err = s.ensureOwnerLibraryActive(ctx, claims, libraryID); err != nil {
		return models.Supplier{}, err
	}
...
func (s *SupplierService) Create(ctx context.Context, claims *auth.Claims, input models.SupplierInput) (models.Supplier, error) {
	...
	if err = normalizeSupplier(&input, false); err != nil {
		return models.Supplier{}, err
	}
	...
	if err = s.repository.Create(ctx, supplier, strings.ToLower(supplier.Name), claims.Subject, auditID, snapshot); err != nil {
		return models.Supplier{}, err
	}
	return supplier, nil
}
...
func (s *SupplierService) Update(ctx context.Context, claims *auth.Claims, id string, input models.SupplierInput) (models.Supplier, error) {
	...
	if input.LibraryID != "" && strings.TrimSpace(input.LibraryID) != existing.LibraryID {
		return models.Supplier{}, ErrBookForbidden
	}
	if err = normalizeSupplier(&input, true); err != nil {
		return models.Supplier{}, err
	}
	updated := existing
	...
	oldValues, err := supplierSnapshot(existing)
	if err != nil {
		return models.Supplier{}, err
	}
	newValues, err := supplierSnapshot(updated)
	if err != nil {
		return models.Supplier{}, err
	}
	_, auditID, err := supplierIDs()
	if err != nil {
		return models.Supplier{}, err
	}
...
func (s *SupplierService) changeStatus(ctx context.Context, claims *auth.Claims, id string, version int,
	...
	if supplier.Status != expected {
		return repositories.ErrSupplierState
	}
	_, auditID, err := supplierIDs()
	if err != nil {
		return err
...
func supplierSnapshot(supplier models.Supplier) (string, error) {
	payload, err := json.Marshal(struct {
		...
		Status      models.SupplierStatus `json:"status"`
