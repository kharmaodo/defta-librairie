...
func TestCashRegisterLifecycleIsolationAndAudit(t *testing.T) {
	...
	if _, err = service.Create(context.Background(), ownerOne, models.CashRegisterInput{Name: "CAISSE PRINCIPALE"}); !errors.Is(err, repositories.ErrCashRegisterConflict) {
	...
	if err != nil || register.Version != 2 {
		t.Fatalf("update register=%+v err=%v", register, err)
	}
	...
	if err = service.Disable(context.Background(), ownerOne, register.ID, 2); err != nil {
		t.Fatalf("disable: %v", err)
	}
	if err = service.Reactivate(context.Background(), ownerOne, register.ID, 3); err != nil {
	...
	if err != nil || total != 1 || len(results) != 1 {
		t.Fatalf("list total=%d results=%+v err=%v", total, results, err)
	}
	var audits int
...
func TestCashRegisterValidation(t *testing.T) {
	if _, _, err := normalizeCashRegisterName("x"); !errors.Is(err, ErrInvalidCashRegister) {
		t.Fatalf("short name: %v", err)
	}
	if _, _, err := normalizeCashRegisterName(strings.Repeat("x", 81)); !errors.Is(err, ErrInvalidCashRegister) {
