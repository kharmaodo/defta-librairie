...
func TestReturnSettlementLifecycleBalanceIsolationAndAudit(t *testing.T) {
	db, err := sql.Open("sqlite3", filepath.Join(t.TempDir(), "settlements.db")+"?_foreign_keys=on")
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = db.Exec(`CREATE TABLE customer_returns(id TEXT PRIMARY KEY,library_id TEXT,resolution TEXT,status TEXT,total_amount REAL,UNIQUE(id,library_id));
		...
		INSERT INTO customer_returns VALUES('return-1','library-1','REFUND','COMPLETED',3000);INSERT INTO customer_returns VALUES('return-2','library-2','REFUND','COMPLETED',1000);INSERT INTO customer_returns VALUES('credit-1','library-1','CREDIT_NOTE','COMPLETED',500);`)
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.Exec(`ALTER TABLE customer_returns ADD COLUMN sale_id TEXT;
        UPDATE customer_returns SET sale_id='sale-'||library_id;
        CREATE TABLE sales(id TEXT,library_id TEXT,status TEXT);
        INSERT INTO sales VALUES('sale-library-1','library-1','CONFIRMED'),('sale-library-2','library-2','CONFIRMED');
        CREATE TABLE payments(sale_id TEXT,status TEXT,amount REAL);
        INSERT INTO payments VALUES('sale-library-1','RECORDED',3000),('sale-library-2','RECORDED',1000);`)
	if err != nil {
		t.Fatal(err)
	}
	service := NewReturnSettlementService(repositories.NewReturnSettlementRepository(db))
	...
	first, err := service.Create(context.Background(), owner, "return-1", models.ReturnSettlementInput{Method: models.ReturnSettlementMethodCash, Amount: 1000})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	balance, err := service.Balance(context.Background(), owner, "return-1")
	if err != nil || balance.RemainingAmount != 2000 || balance.SettlementStatus != "PARTIALLY_SETTLED" {
		t.Fatalf("balance=%+v err=%v", balance, err)
	}
	if _, err = service.Balance(context.Background(), other, "return-1"); !errors.Is(err, repositories.ErrReturnSettlementReturnNotFound) {
		t.Fatalf("cross-library balance: %v", err)
	}
	if _, err = service.Create(context.Background(), owner, "return-1", models.ReturnSettlementInput{Method: models.ReturnSettlementMethodCash, Amount: 2500}); !errors.Is(err, repositories.ErrReturnSettlementOverpaid) {
		t.Fatalf("over settlement: %v", err)
	}
	if _, err = service.Create(context.Background(), owner, "credit-1", models.ReturnSettlementInput{Method: models.ReturnSettlementMethodCash, Amount: 500}); !errors.Is(err, repositories.ErrReturnSettlementUnavailable) {
		t.Fatalf("wrong resolution: %v", err)
	}
	first, err = service.Void(context.Background(), owner, first.ID, models.ReturnSettlementVoidInput{Version: first.Version, Reason: "Erreur de saisie"})
	if err != nil || first.Status != models.ReturnSettlementStatusVoided {
		t.Fatalf("void=%+v err=%v", first, err)
	}
	balance, err = service.Balance(context.Background(), owner, "return-1")
	if err != nil || balance.RemainingAmount != 3000 || balance.SettlementStatus != "PENDING" {
		t.Fatalf("void balance=%+v err=%v", balance, err)
	}
	var audits int
	_ = db.QueryRow(`SELECT COUNT(*) FROM audit_logs WHERE action IN('ISSUE_RETURN_SETTLEMENT','VOID_RETURN_SETTLEMENT')`).Scan(&audits)
	if audits != 2 {
