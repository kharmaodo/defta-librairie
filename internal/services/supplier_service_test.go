...
func TestSupplierLifecycleIsolationAndAudit(t *testing.T) {
	...
	if err != nil { t.Fatalf("create supplier: %v", err) }
	...
	if _, err = service.Create(context.Background(), ownerOne, models.SupplierInput{Name:"éditions defta"});
		!errors.Is(err, repositories.ErrSupplierConflict) { t.Fatalf("duplicate supplier: %v", err) }
	...
	if err != nil || supplier.Version != 2 { t.Fatalf("update supplier=%+v err=%v", supplier, err) }
	if _, err = service.Update(context.Background(), ownerOne, supplier.ID, models.SupplierInput{Name:"Ancienne version", Version:1});
		!errors.Is(err, repositories.ErrSupplierVersion) { t.Fatalf("expected version conflict, got %v", err) }
	if err = service.Disable(context.Background(), ownerOne, supplier.ID, 2); err != nil { t.Fatalf("disable supplier: %v", err) }
	if err = service.Reactivate(context.Background(), ownerOne, supplier.ID, 3); err != nil { t.Fatalf("reactivate supplier: %v", err) }

	all, total, err := service.List(context.Background(), root, "", "Defta", "", 0, 30)
	if err != nil || total != 1 || len(all) != 1 { t.Fatalf("root list total=%d suppliers=%+v err=%v", total, all, err) }
	ownerTwoList, total, err := service.List(context.Background(), ownerTwo, "", "", "", 0, 30)
	if err != nil || total != 0 || len(ownerTwoList) != 0 { t.Fatalf("owner isolation total=%d suppliers=%+v err=%v", total, ownerTwoList, err) }
	var audits int
...
func TestSupplierValidation(t *testing.T) {
	...
	if err := normalizeSupplier(&invalid[0], false); !errors.Is(err, ErrInvalidSupplier) { t.Fatalf("short name: %v", err) }
	if err := normalizeSupplier(&invalid[1], false); !errors.Is(err, ErrInvalidSupplier) { t.Fatalf("email: %v", err) }
	if err := normalizeSupplier(&invalid[2], true); !errors.Is(err, ErrInvalidSupplier) { t.Fatalf("version: %v", err) }
}
