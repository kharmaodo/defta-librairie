...
func (r *SupplierReturnRepository) Ship(ctx context.Context, id, libraryID string, version int, actor, now string) (models.SupplierReturn, error) {
 ...
 if err!=nil{return models.SupplierReturn{},err}
 n,err:=result.RowsAffected();if err!=nil{return models.SupplierReturn{},err}
 if n!=1 {
   q:=`SELECT status FROM supplier_returns WHERE id=?`;a:=[]interface{}{id}
   if libraryID!=""{q+=` AND library_id=?`;a=append(a,libraryID)}
   var status string
   err=tx.QueryRowContext(ctx,q,a...).Scan(&status)
   if errors.Is(err,sql.ErrNoRows){return models.SupplierReturn{},ErrSupplierReturnNotFound}
   if err!=nil{return models.SupplierReturn{},err}
   if status!="DRAFT"{return models.SupplierReturn{},ErrSupplierReturnState}
   return models.SupplierReturn{},ErrSupplierReturnConflict
 }
 var scope,reference string
 if err=tx.QueryRowContext(ctx,`SELECT library_id,reference FROM supplier_returns WHERE id=?`,id).Scan(&scope,&reference);err!=nil{return models.SupplierReturn{},err}
 ...
 if err!=nil{return models.SupplierReturn{},err}
 type line struct{book int64;quantity int}
 lines:=[]line{}
 for rows.Next(){var l line;if err=rows.Scan(&l.book,&l.quantity);err!=nil{rows.Close();return models.SupplierReturn{},err};lines=append(lines,l)}
 err=rows.Err();rows.Close();if err!=nil{return models.SupplierReturn{},err}
 if len(lines)==0{return models.SupplierReturn{},ErrSupplierReturnLine}
 for _,l:=range lines {
   var before int
   err=tx.QueryRowContext(ctx,`SELECT quantity FROM book_inventory WHERE book_id=? AND library_id=?`,l.book,scope).Scan(&before)
   if errors.Is(err,sql.ErrNoRows){return models.SupplierReturn{},ErrSupplierReturnLine}
   if err!=nil{return models.SupplierReturn{},err}
   if before<l.quantity{return models.SupplierReturn{},ErrSupplierReturnStock}
   ...
   if err!=nil{return models.SupplierReturn{},err}
   n,err=result.RowsAffected();if err!=nil{return models.SupplierReturn{},err};if n!=1{return models.SupplierReturn{},ErrSupplierReturnConflict}
   movement,err:=identity.NewID();if err!=nil{return models.SupplierReturn{},err}
   ...
   if err!=nil{return models.SupplierReturn{},err}
   audit,err:=identity.NewID();if err!=nil{return models.SupplierReturn{},err}
   payload,err:=json.Marshal(map[string]interface{}{"supplierReturnId":id,"quantityBefore":before,"quantityAfter":after,"quantityDelta":-l.quantity,"movementType":"EXIT"});if err!=nil{return models.SupplierReturn{},err}
 ...
 audit,err:=identity.NewID();if err!=nil{return models.SupplierReturn{},err}
 payload,err:=json.Marshal(map[string]interface{}{"reference":reference,"from":"DRAFT","to":"SHIPPED","version":version+1});if err!=nil{return models.SupplierReturn{},err}
 ...
 if err!=nil{return models.SupplierReturn{},err}
 if err=tx.Commit();err!=nil{return models.SupplierReturn{},err}
