...
func (r *PurchaseRepository) List(ctx context.Context, libraryID string, filter models.PurchaseFilter,
	...
	if err = rows.Close(); err != nil {
		return nil, 0, fmt.Errorf("close purchases: %w", err)
	}
	for index := range purchases {
		if purchases[index].Lines, err = r.listLines(ctx, purchases[index].ID); err != nil {
			return nil, 0, err
		}
	}
...
func scanPurchase(row purchaseScanner) (models.Purchase, error) {
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("scan purchase: %w", err)
	}
	return purchase, nil
}
...
func (r *PurchaseRepository) listLines(ctx context.Context, purchaseID string) ([]models.PurchaseLine, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list purchase lines: %w", err)
	}
	defer rows.Close()
...
func (r *PurchaseRepository) Create(ctx context.Context, purchase models.Purchase, inputs []models.PurchaseLineInput,
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("begin purchase creation: %w", err)
	}
	defer tx.Rollback()
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("insert purchase: %w", err)
	}
	...
	if err = tx.Commit(); err != nil {
		return models.Purchase{}, fmt.Errorf("commit purchase creation: %w", err)
	}
...
func requireActiveSupplier(ctx context.Context, tx *sql.Tx, supplierID, libraryID string) error {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return ErrPurchaseSupplier
	}
	if err != nil {
		return fmt.Errorf("read purchase supplier: %w", err)
	}
	return nil
}
...
func writePurchaseLines(ctx context.Context, tx *sql.Tx, purchaseID, libraryID string,
	...
	for index, input := range inputs {
		...
		if errors.Is(err, sql.ErrNoRows) {
			return nil, 0, ErrPurchaseBook
		}
		if err != nil {
			return nil, 0, fmt.Errorf("read purchase book: %w", err)
		}
		...
		if err != nil {
			return nil, 0, fmt.Errorf("insert purchase line: %w", err)
		}
		total += lineTotal
		lines = append(lines, line)
	}
...
func (r *PurchaseRepository) Update(ctx context.Context, id, libraryID, supplierID string,
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return models.Purchase{}, fmt.Errorf("begin purchase update: %w", err)
	}
	defer tx.Rollback()
	...
	if err = tx.QueryRowContext(ctx, query, args...).Scan(&actualLibrary, &status, &version); errors.Is(err, sql.ErrNoRows) {
		...
		return models.Purchase{}, fmt.Errorf("read purchase before update: %w", err)
	}
	if status != models.PurchaseStatusDraft {
		return models.Purchase{}, ErrPurchaseState
	}
	if version != expectedVersion {
		return models.Purchase{}, ErrPurchaseConflict
	}
	if err = requireActiveSupplier(ctx, tx, supplierID, actualLibrary); err != nil {
		return models.Purchase{}, err
	}
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("update purchase: %w", err)
	}
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO audit_logs(id,actor_user_id,action,resource_type,resource_id,
		...
		actorID, id, string(payload), now); err != nil {
		return models.Purchase{}, fmt.Errorf("audit purchase update: %w", err)
	}
	if err = tx.Commit(); err != nil {
		return models.Purchase{}, fmt.Errorf("commit purchase update: %w", err)
	}
...
func (r *PurchaseRepository) Delete(ctx context.Context, id, libraryID string, expectedVersion int,
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin purchase deletion: %w", err)
	}
	defer tx.Rollback()
	...
	if err = tx.QueryRowContext(ctx, query, args...).Scan(&reference, &status, &total, &version); errors.Is(err, sql.ErrNoRows) {
		...
		return fmt.Errorf("read purchase before deletion: %w", err)
	}
	if status != models.PurchaseStatusDraft {
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO audit_logs(id,actor_user_id,action,resource_type,resource_id,
		...
		actorID, id, string(payload), now); err != nil {
		return fmt.Errorf("audit purchase deletion: %w", err)
	}
	return tx.Commit()
}
...
func (r *PurchaseRepository) Transition(ctx context.Context, id, libraryID, actorID string,
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return models.Purchase{}, fmt.Errorf("begin purchase transition: %w", err)
	}
	defer tx.Rollback()
	...
	if err = tx.QueryRowContext(ctx, query, args...).Scan(&actualLibrary, &reference, &current, &version); errors.Is(err, sql.ErrNoRows) {
		...
		return models.Purchase{}, fmt.Errorf("read purchase before transition: %w", err)
	}
	if version != expectedVersion {
		return models.Purchase{}, ErrPurchaseConflict
	}
	if current != models.PurchaseStatusDraft {
		return models.Purchase{}, ErrPurchaseState
	}
	...
	if target == models.PurchaseStatusReceived {
		...
		if err = rows.Close(); err != nil {
			return models.Purchase{}, fmt.Errorf("close purchase receipt lines: %w", err)
		}
		...
		for index, line := range lines {
			...
			if affected, rowsErr := result.RowsAffected(); rowsErr != nil || affected != 1 {
				return models.Purchase{}, ErrInventoryConflict
			}
			...
			if err != nil {
				return models.Purchase{}, fmt.Errorf("insert purchase inventory movement: %w", err)
			}
			...
			if err != nil {
				return models.Purchase{}, fmt.Errorf("audit purchase inventory: %w", err)
			}
		}
	}
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("transition purchase: %w", err)
	}
	if affected, rowsErr := result.RowsAffected(); rowsErr != nil || affected != 1 {
		return models.Purchase{}, ErrPurchaseConflict
	}
	...
	if err != nil {
		return models.Purchase{}, fmt.Errorf("audit purchase transition: %w", err)
	}
	if err = tx.Commit(); err != nil {
		return models.Purchase{}, fmt.Errorf("commit purchase transition: %w", err)
	}
	return r.Find(ctx, id, libraryID)
}
