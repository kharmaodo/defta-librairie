...
func (r *SupplierReturnRepository) List(ctx context.Context, libraryID string,
	...
	if filter.PurchaseID != "" { where += " AND purchase_id=?"; args = append(args, filter.PurchaseID) }
	...
	var total int
	if err := r.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM supplier_returns"+where, args...).Scan(&total); err != nil { return nil, 0, err }
	...
	for rows.Next() { value, scanErr := scanSupplierReturn(rows); if scanErr != nil { return nil, 0, scanErr }; value.Lines, scanErr = r.listLines(ctx, value.ID); if scanErr != nil { return nil, 0, scanErr }; values = append(values, value) }
...
func (r *SupplierReturnRepository) Find(ctx context.Context, id, libraryID string) (models.SupplierReturn, error) {
	...
	if errors.Is(err, sql.ErrNoRows) { return value, ErrSupplierReturnNotFound }
...
func (r *SupplierReturnRepository) listLines(ctx context.Context, id string) ([]models.SupplierReturnLine, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id,return_id,purchase_line_id,book_id,title_snapshot,quantity,unit_cost,line_total,created_at FROM supplier_return_lines WHERE return_id=? ORDER BY id`, id)
	if err != nil { return nil, err }
	defer rows.Close()
	values := []models.SupplierReturnLine{}
	for rows.Next() { var value models.SupplierReturnLine; if err = rows.Scan(&value.ID,&value.ReturnID,&value.PurchaseLineID,&value.BookID,&value.Title,&value.Quantity,&value.UnitCost,&value.LineTotal,&value.CreatedAt);err!=nil{return nil,err};values=append(values,value) }
...
func (r *SupplierReturnRepository) Create(ctx context.Context, value models.SupplierReturn,
	...
	tx, err := r.db.BeginTx(ctx, nil); if err != nil { return value, err }; defer tx.Rollback()
	var supplierID string
	if err = tx.QueryRowContext(ctx, `SELECT supplier_id FROM purchases WHERE id=? AND library_id=? AND status='RECEIVED'`, value.PurchaseID, value.LibraryID).Scan(&supplierID); errors.Is(err,sql.ErrNoRows){return value,ErrSupplierReturnPurchase}else if err!=nil{return value,err}
	...
	if err != nil { return value, mapSupplierReturnError(err) }
	lines,total,err:=writeSupplierReturnLines(ctx,tx,value.ID,value.PurchaseID,inputs,lineIDs,now);if err!=nil{return value,err}
	if _,err=tx.ExecContext(ctx,"UPDATE supplier_returns SET total_amount=? WHERE id=?",total,value.ID);err!=nil{return value,err}
	...
	if _,err=tx.ExecContext(ctx,`INSERT INTO audit_logs(id,actor_user_id,action,resource_type,resource_id,new_values,success,created_at) VALUES(?,?,'CREATE_SUPPLIER_RETURN','SUPPLIER_RETURN',?,?,1,?)`,auditID,value.CreatedBy,value.ID,string(payload),now);err!=nil{return value,err}
	if err=tx.Commit();err!=nil{return value,err};value.Status=models.SupplierReturnStatusDraft;value.TotalAmount=total;value.Version=1;value.CreatedAt=now;value.UpdatedAt=now;value.Lines=lines;return value,nil
}
...
func writeSupplierReturnLines(ctx context.Context, tx *sql.Tx, returnID,purchaseID string, inputs []models.SupplierReturnLineInput,lineIDs []string,now string)([]models.SupplierReturnLine,float64,error){
	values:=make([]models.SupplierReturnLine,0,len(inputs));var total float64
	for index,input:=range inputs{var bookID int64;var title string;var unitCost float64;err:=tx.QueryRowContext(ctx,`SELECT book_id,title_snapshot,unit_cost FROM purchase_lines WHERE id=? AND purchase_id=?`,input.PurchaseLineID,purchaseID).Scan(&bookID,&title,&unitCost);if errors.Is(err,sql.ErrNoRows){return nil,0,ErrSupplierReturnLine};if err!=nil{return nil,0,err};lineTotal:=unitCost*float64(input.Quantity);value:=models.SupplierReturnLine{ID:lineIDs[index],ReturnID:returnID,PurchaseLineID:input.PurchaseLineID,BookID:bookID,Title:title,Quantity:input.Quantity,UnitCost:unitCost,LineTotal:lineTotal,CreatedAt:now};if _,err=tx.ExecContext(ctx,`INSERT INTO supplier_return_lines(id,return_id,purchase_line_id,book_id,title_snapshot,quantity,unit_cost,line_total,created_at) VALUES(?,?,?,?,?,?,?,?,?)`,value.ID,value.ReturnID,value.PurchaseLineID,value.BookID,value.Title,value.Quantity,value.UnitCost,value.LineTotal,value.CreatedAt);err!=nil{return nil,0,mapSupplierReturnError(err)};values=append(values,value);total+=lineTotal}
