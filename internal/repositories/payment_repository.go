...
func (r *PaymentRepository) SaleLibrary(ctx context.Context, saleID, libraryID string) (string, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return "", ErrPaymentSaleNotFound
	}
	if err != nil {
		return "", fmt.Errorf("find payment sale: %w", err)
	}
	return result, nil
}
...
func paymentWhere(saleID, libraryID string, filter models.PaymentFilter) (string, []interface{}) {
	...
	if filter.Method != "" {
		conditions, args = append(conditions, "method=?"), append(args, filter.Method)
	}
	if filter.Status != "" {
...
func (r *PaymentRepository) Find(ctx context.Context, id, libraryID string) (models.Payment, error) {
	...
	if err != nil {
		return models.Payment{}, fmt.Errorf("find payment: %w", err)
	}
	return payment, nil
}
...
func (r *PaymentRepository) Balance(ctx context.Context, saleID, libraryID string) (models.SalePaymentBalance, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return models.SalePaymentBalance{}, ErrPaymentSaleNotFound
	}
	if err != nil {
		return models.SalePaymentBalance{}, fmt.Errorf("find payment balance: %w", err)
	}
	return balance, nil
}
...
func (r *PaymentRepository) Create(ctx context.Context, payment models.Payment, auditID, snapshot string) error {
	...
	if err != nil {
		return fmt.Errorf("begin payment creation: %w", err)
	}
	defer tx.Rollback()
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO audit_logs
		...
		payment.ID, snapshot, payment.CreatedAt); err != nil {
		return fmt.Errorf("audit payment creation: %w", err)
	}
	return tx.Commit()
}
...
func (r *PaymentRepository) Void(ctx context.Context, payment models.Payment, expectedVersion int,
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin payment void: %w", err)
	}
	defer tx.Rollback()
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO audit_logs
		...
		oldValues, newValues, now); err != nil {
		return fmt.Errorf("audit payment void: %w", err)
	}
	return tx.Commit()
}
