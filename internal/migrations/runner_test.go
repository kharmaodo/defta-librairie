...
func TestRunMigratesLegacyCatalogueAndIsIdempotent(t *testing.T) {
	...
	if migrationsCount != 28 {
		t.Fatalf("expected 28 migrations, got %d", migrationsCount)
	}
	...
	var customerReturnObjects int
	if err = db.QueryRow(`
		SELECT COUNT(*) FROM sqlite_master
		WHERE name IN ('customer_returns','customer_return_lines','return_settlements','customer_return_balances')
		  AND type IN ('table','view')
	`).Scan(&customerReturnObjects); err != nil {
		t.Fatalf("inspect customer return objects: %v", err)
	}
	if customerReturnObjects != 4 {
		t.Fatalf("expected customer return tables and balance view, got %d", customerReturnObjects)
	}
	var supplierReturnTables int
	if err = db.QueryRow(`
		SELECT COUNT(*) FROM sqlite_master
		WHERE type='table' AND name IN ('supplier_returns','supplier_return_lines')
	`).Scan(&supplierReturnTables); err != nil {
		t.Fatalf("inspect supplier return tables: %v", err)
	}
	if supplierReturnTables != 2 {
		t.Fatalf("expected supplier return tables, got %d", supplierReturnTables)
	}

	var coverTables int
	if err = db.QueryRow(`
		SELECT COUNT(*) FROM sqlite_master
		WHERE type='table' AND name IN ('book_covers','cover_processing_outbox')
	`).Scan(&coverTables); err != nil {
		t.Fatalf("inspect cover and outbox tables: %v", err)
	}
	if coverTables != 2 {
	...
	var coverCleanupTables int
	if err = db.QueryRow(`
		SELECT COUNT(*) FROM sqlite_master
		WHERE type='table' AND name='cover_object_cleanup_jobs'
	`).Scan(&coverCleanupTables); err != nil {
		t.Fatalf("inspect cover cleanup table: %v", err)
	}
