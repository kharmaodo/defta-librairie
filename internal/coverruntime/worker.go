...
// RunWorker supervise le traitement des couvertures jusqu'à l'annulation du
// contexte. Une indisponibilité NATS est tolérée et suivie d'une reconnexion.
func RunWorker(
	ctx context.Context,
	cfg *config.Config,
	db *sql.DB,
	...
	if ctx == nil || cfg == nil || db == nil || logger == nil {
		return
	}
	...
	for ctx.Err() == nil {
		...
		if connectErr != nil {
			...
			if !wait(ctx, workerReconnectDelay) {
				return
			}
			continue
		}
		...
		if !reconnect || !wait(ctx, workerReconnectDelay) {
			return
		}
	}
}
